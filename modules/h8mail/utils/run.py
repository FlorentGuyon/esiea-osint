...
def target_factory(targets, user_args, verbose = True):
    """
    ...
    for counter, t in enumerate(targets):
        ...
        if not user_args.skip_defaults:
            scylla_up = check_scylla_online(verbose = verbose)
            if scylla_up:
...
def h8mail(user_args, verbose = True):
    """
    ...
    for arg in user_args.user_targets:
        user_stdin_target = fetch_emails(arg, user_args, verbose = verbose)
    ...
    # Launch
    breached_targets = target_factory(targets, user_args, verbose = verbose)
